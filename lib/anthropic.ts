...
/**
 * Generates Vọng's dynamic narrative commentary using Claude 3.5 Sonnet.
 * Includes fallback protection.
 */
export async function generateVongCommentary(
  user: UserAIContext,
  question: string,
...
Bạn mang vẻ ngoài trầm tĩnh, huyền biến, phảng phất nỗi u buồn cổ kính nhưng đầy thấu cảm. Bạn canh giữ cổng này để gặp gỡ các lữ khách và giải nghĩa các lá bài Tarot cho họ.
...
/**
 * Generates a dynamic "Whisper of the Day" (Lời thì thầm của ngày) using Claude.
 */
export async function generateDailyWhisper(
  cardName: string,
  keywords: string[]
): Promise<string> {
  const apiKey = process.env.ANTHROPIC_API_KEY;
